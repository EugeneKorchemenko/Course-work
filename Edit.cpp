...
void chooseKindOfEditing(vector<TypeOfWork>& vector_of_works, int choice)
{
	...
	while (ed != 0)
	{
		...
		switch (ed)
		{
		case 1: // измен€ет весь проект целиком
			editWholeProject(vector_of_works, choice);
			break;
		case 2:// мен€ет им€ проекта
			changeProjectName(vector_of_works, choice);
			break;
		case 3:// измен€ет выбранный тип работ в проекте
			editEmployeeInProject(vector_of_works, choice);
			break;
		case 4:// добавл€ет сотрудников в существующий проект
			addEmployeesInProject(vector_of_works, choice);
			break;
		case 5:// удал€ет сотрудников в существующий проект
...
void editWholeProject(vector<TypeOfWork>& vector_of_works, int choice)
{
	...
	if (approve == 1)
	{
		...
		{
			inputInformation(work_temp, name, vector_of_works);// перегруженна€ функци€. Pеализаци€ находитс€ в Write.h
...
void deleteAccount(vector <Account>& vector_of_accaunts, string login)
{
	...
	if (approve == 1)
	{
		...
		clearConsole();
		if (vector_of_accaunts.at(choice).login == login) // если удал€ем свой аккаунт выходим в меню
			autorisatie();
	}
}
...
bool inputNewPassword(vector <Account>& vector_of_accaunts, Account& account_temp, string& password1, string& password2)
{
	...
	while (flag == false)
	{
		...
		flag = compareInputData(account_temp, vector_of_accaunts); // провер€ем правильно ли введен текущий пароль
		if (flag == true)
		{
			...
			clearConsole();
			flag = isPasswordSuitable(account_temp, password1, password2);  // провер€ем что пароли совпадают и соответсвуют нашим стандартам ( X+1 и более 5 символов) flag == true
			if (flag == false)
			{
				cout << endl << O_TO_TURN_BACK << endl;
				approve = _getch();
...
void approveAllApplications(vector <Account>& vector_unverified_accounts, vector <Account>& vector_of_accaunts)
{
	...
	if (approve == 1)
	{
		for (int i = 0; i < vector_of_accaunts.size(); i++) // мен€ем статус в векторе всех аккаунтов
		{
...
void approveOneApplication(vector <Account>& vector_unverified_accounts, vector <Account>& vector_of_accaunts)
{
	...
	if (approve == 1)
	{
		...
		vector_unverified_accounts.erase(vector_unverified_accounts.cbegin() + (choice - 1)); // удал€ем из списка неподтвержденных аккаунтов
		writeFileAccounts(vector_of_accaunts); // сохран€ем измененив файл
	}
	clearConsole();
}
